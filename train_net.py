...
"""
ConsistencyDet Training Script.
...
from ConsistencyDet import ConsistencyDetDatasetMapper, add_ConsistencyDet_config, ConsistencyDetWithTTA
from ConsistencyDet.util.model_ema import add_model_ema_configs, may_build_model_ema, may_get_ema_checkpointer, EMAHook, \
    apply_model_ema_and_restore, EMADetectionCheckpointer


class Trainer(DefaultTrainer):
    """ Extension of the Trainer class adapted to ConsistencyDet. """
    ...
    def __init__(self, cfg):
        ...
        model = self.build_model(cfg)  # ConsistencyDet model
    ...
    def build_train_loader(cls, cfg):
        mapper = ConsistencyDetDatasetMapper(cfg, is_train=True)
    ...
    def test_with_TTA(cls, cfg, model):
        ...
        model = ConsistencyDetWithTTA(cfg, model)
        evaluators = [
            cls.build_evaluator(
...
def setup(args):
    ...
    """
    cfg = get_cfg()
    add_ConsistencyDet_config(cfg)
    add_model_ema_configs(cfg)  # 感觉不用加了
...
def main(args):
    ...
    #print(cfg.MODEL.ConsistencyDet.CONSISTENCY_WEIGHT)
    trainer = Trainer(cfg)
