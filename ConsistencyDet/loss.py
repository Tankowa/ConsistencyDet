...
class SetCriterionDynamicK(nn.Module):
    """ This class computes the loss for ConsistencyDet.
    ...
    def __init__(self, cfg, num_classes, matcher, weight_dict, eos_coef, losses, use_focal):
        ...
        if self.use_focal:
            self.focal_loss_alpha = cfg.MODEL.ConsistencyDet.ALPHA
            self.focal_loss_gamma = cfg.MODEL.ConsistencyDet.GAMMA
        else:
...
class HungarianMatcherDynamicK(nn.Module):
    ...
    def __init__(self, cfg, cost_class: float = 1, cost_bbox: float = 1, cost_giou: float = 1, cost_mask: float = 1, use_focal: bool = False):
        ...
        self.use_focal = use_focal
        self.use_fed_loss = cfg.MODEL.ConsistencyDet.USE_FED_LOSS
        self.ota_k = cfg.MODEL.ConsistencyDet.OTA_K
        if self.use_focal:
            self.focal_loss_alpha = cfg.MODEL.ConsistencyDet.ALPHA
            self.focal_loss_gamma = cfg.MODEL.ConsistencyDet.GAMMA
